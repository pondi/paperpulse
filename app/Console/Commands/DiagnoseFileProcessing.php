...
class DiagnoseFileProcessing extends Command
{
    ...
    protected function checkEnvironmentConfiguration()
    {
        ...
        $configs = [
            ...
            'AI Provider' => config('ai.provider', 'not set'),
            'Textract Region' => config('ai.ocr.providers.textract.region', 'not set'),
        ];
        ...
        // Validate required configuration values (avoid direct env() lookups)
        $requiredConfig = [
            'S3 Key (paperpulse)' => config('filesystems.disks.paperpulse.key'),
            'S3 Secret (paperpulse)' => config('filesystems.disks.paperpulse.secret'),
            'S3 Region (paperpulse)' => config('filesystems.disks.paperpulse.region'),
            'S3 Bucket (paperpulse)' => config('filesystems.disks.paperpulse.bucket'),
            'S3 Key (pulsedav)' => config('filesystems.disks.pulsedav.key'),
            'S3 Secret (pulsedav)' => config('filesystems.disks.pulsedav.secret'),
            'S3 Region (pulsedav)' => config('filesystems.disks.pulsedav.region'),
            'S3 Bucket (pulsedav)' => config('filesystems.disks.pulsedav.bucket'),
            'Textract Key' => config('ai.ocr.providers.textract.key'),
            'Textract Secret' => config('ai.ocr.providers.textract.secret'),
            'Textract Region' => config('ai.ocr.providers.textract.region'),
            'OpenAI API Key' => config('ai.providers.openai.api_key'),
        ];

        $missing = collect($requiredConfig)
    ...
    protected function testServiceConnections()
    {
        ...
        try {
            ...
            $textractRegion = config('ai.ocr.providers.textract.region');
            ...
            if ($fileRecord) {
                ...
                $this->table(
                    ...
                    [
                        ...
                        ['Original Filename', $fileRecord->original_filename],
