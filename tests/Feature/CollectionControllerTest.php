...
describe('Collection API Controller', function () {
    test('api index returns paginated collections', function () {
        Collection::factory()->count(3)->create(['user_id' => $this->user->id]);
    ...
    test('api can create collection', function () {
        $response = $this->actingAs($this->user, 'sanctum')
        ...
        $response->assertJsonPath('data.name', 'API Collection');
    });

    test('api can show collection', function () {
        $collection = Collection::factory()->create(['user_id' => $this->user->id]);
        ...
        $response->assertOk();
        $response->assertJsonPath('data.collection.name', $collection->name);
        $response->assertJsonPath('data.is_owner', true);
    });

    test('api can update collection', function () {
        $collection = Collection::factory()->create(['user_id' => $this->user->id]);
        ...
        $response->assertOk();
        $response->assertJsonPath('data.name', 'Updated via API');
    });

    test('api can delete collection', function () {
        $collection = Collection::factory()->create(['user_id' => $this->user->id]);
    ...
    test('api can add files to collection', function () {
        $collection = Collection::factory()->create(['user_id' => $this->user->id]);
        $files = File::factory()->count(2)->create(['user_id' => $this->user->id]);

        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson("/api/v1/collections/{$collection->id}/files", [
                'file_ids' => $files->pluck('id')->toArray(),
            ]);
    ...
    test('api can share collection', function () {
        $collection = Collection::factory()->create(['user_id' => $this->user->id]);
        $targetUser = User::factory()->create();

        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson("/api/v1/collections/{$collection->id}/share", [
                'email' => $targetUser->email,
                'permission' => 'edit',
            ]);
    ...
    test('api returns 403 for unauthorized access', function () {
        $otherUser = User::factory()->create();
        $collection = Collection::factory()->create(['user_id' => $otherUser->id]);
